...
function superShootSoccer(players) {
  ...
  for(let i=0; i<players.length; i++){
    arrayPower.push(players[i].Power)
  }

  for(let i=0; i<arrayPower.length; i++){
    for(let j=0; j<arrayPower.length-1; j++){
      if(arrayPower[j]<arrayPower[j+1]){
        var temp = arrayPower[j]
        arrayPower[j] = arrayPower[j+1]
        arrayPower[j+1] = temp
      }
    }
  }
  ...
  for(let i=0; i<arrayPower.length; i++){
    for(let j=0; j<players.length; j++){
      if(arrayPower[i] === players[j].Power){
        winnerOrder.push(players[j].Club)
      }
    }
  }
return  {         
          'Juara 1': winnerOrder[0],
          'Juara 2': winnerOrder[1],
          'Juara 3': winnerOrder[2],
          'Juara 4': winnerOrder[3]
        }
}
